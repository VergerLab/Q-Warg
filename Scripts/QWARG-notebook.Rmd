...
wdta <- merge(wdta, thresholds, by = "med_rep", all.x = TRUE)
...
wdta$cellalive <- ifelse(wdta$meanViability > wdta$huang & wdta$Circularity > 0.9, "yes", "no")
...
for (condition in unique(wdta$med_rep)) {
  ...
  thresholds_row <- thresholds[thresholds$med_rep == condition, ]
  ...
  if (nrow(thresholds_row) > 0) {
    p <- p +
      geom_hline(yintercept = thresholds_row$huang, color = "purple", linetype = "solid") +
      annotate("text", x = Inf, y = thresholds_row$huang,
               label = paste("Huang =", round(thresholds_row$huang, 2)), 
               vjust=-0.5, hjust=1.1, color="purple", size = 2)
  }
  ...
  if (nrow(thresholds_row) > 0) {
    q <- q +
      geom_vline(xintercept = thresholds_row$huang, color = "purple", linetype = "solid")
  }
