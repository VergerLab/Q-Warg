...
calculate_thresholds <- function(data) {
  ...
  maxentropy <- tryCatch(
    auto_thresh(scaled_viability, method = "MaxEntropy"),
    error = function(e) {
      message("MaxEntropy method failed to find threshold. Use another method instead.")
    }
  )
  ...
  original_maxentropy <- maxentropy * (max(data$meanViability) - min(data$meanViability)) / 255 + min(data$meanViability)
  
  return(c(huang = original_huang, triangle = original_triangle, maxentropy = original_maxentropy))
}
  ...
  fluidRow(
    column(width = 4,
      wellPanel(
        ...
        radioButtons("threshold", "Threshold:", choices = c("Huang", "Triangle", "MaxEntropy", "Manual")),
...
observeEvent(input$apply_thr, {
    ...
    threshold_method <- input$threshold
    selected_value <- NULL
    
    tryCatch({
        if (threshold_method %in% c("Huang", "Triangle", "MaxEntropy")) {
            column_name <- tolower(threshold_method)
            if (column_name %in% names(current_thresholds) && !is.na(current_thresholds[[column_name]][row_index])) {
                selected_value <- current_thresholds[[column_name]][row_index]
            } else {
                stop(paste(threshold_method, "threshold could not be computed"))
            }
        } else {
            selected_value <- as.numeric(input$manual_value)
            if (is.na(selected_value)) stop("Invalid manual value")
        }
        ...
        showNotification(paste("Threshold applied:", threshold_method, "=", round(selected_value, 2)), 
                         type = "message")
    }, error = function(e) {
  ...
  output$via_plot <- renderPlot({
    ...
    p <- p +
      ...
      {
    if (!is.null(current_thresholds) && 
        "maxentropy" %in% names(current_thresholds) && 
        !is.na(current_thresholds$maxentropy)) {
      list(
        geom_hline(yintercept = current_thresholds$maxentropy, color = "green", linetype = "dashed") +
        annotate("text", x = Inf, y = current_thresholds$maxentropy,
               label = paste("MaxEntropy =", round(current_thresholds$maxentropy, 2)), 
               vjust = -0.5, hjust = 1.1, color = "blue", size = 3)
      )
    }
  }
    ...
    if (!is.na(current_thresholds$thr_used)) {
      ...
      method <- ifelse(selected_value == current_thresholds$huang, "Huang",
                       ifelse(selected_value == current_thresholds$triangle, "Triangle",
                              ifelse(selected_value == current_thresholds$maxentropy, "MaxEntropy","Manual")))
...
output$via_density <- renderPlot({
  ...
  p <- p +
    ...
    if (!is.null(current_thresholds) && 
        ...
        !is.na(current_thresholds$maxentropy)) {
      list(
        geom_vline(xintercept = current_thresholds$maxentropy, color = "blue", linetype = "dashed"),
        annotate("text", x = current_thresholds$maxentropy, y = Inf, label = "MaxEntropy", 
                 color = "blue", vjust = 2, hjust = 1.1, angle = 90)
      )
    }
  }
  ...
  if (nrow(thresholds_row) > 0) {
    p <- p +
      ...
      annotate("text", x = Inf, y = thresholds_row$thr_used,
               label = paste("threshold =", round(thresholds_row$thr_used, 2)), 
...
### STATISTICS ###_____________________________________________________

df_list <- list()

for (col in names(livedta)[names(livedta) != "Medium"]) {
  df_temp <- livedta %>% select(Medium, !!col)
...
columns_to_remove <- c("med_rep", "cell", "rep_medium", "Replicate", "cellalive")
...
hsd_list <- lapply(df_list, function(df) {
  HSD.test(aov(value ~ Medium, data = df), "Medium", group = TRUE)
})
...
hsd_df_list <- lapply(hsd_list, function(hsd) {
  data.frame(hsd$groups) %>%
    mutate(Medium = row.names(hsd$groups)) %>%
    select(-value)
})

# Join the data frames from df_list and hsd_df_list
stats_list <- Map(function(df, hsd_df) {
  left_join(df, hsd_df, by = "Medium")
}, df_list, hsd_df_list)

for (i in seq_along(data_summarized_list)) {
  condition <- data_summarized_list[[i]]$Medium
  groups <- hsd_df_list[[i]]$groups[match(condition, hsd_df_list[[i]]$Medium)]
...
combined_list <- lapply(stats_list, function(df) {
  ...
  df$sample <- livedta$cell[1:nrow(df)]
  df$cond_replicate <- livedta$med_rep[1:nrow(df)]
  df$rep <- livedta$Replicate[1:nrow(df)]
...
###### STATS for viability ######_________________________________________________________
chi_results <- chisq.test(table(wdta$Medium, wdta$cellalive))
print(chi_results)

# Pairwise comparisons (if overall test is significant)
pairwise_results <- pairwise.prop.test(table(wdta$Medium, wdta$cellalive), p.adjust.method = "bonferroni")

p_value_matrix <- pairwise_results$p.value

print(p_value_matrix)

### PLOTS ###_____________________________________________________________________________________
...
CW_intensity_size_plot <- livedta %>%
  ...
  geom_text(data = data.frame(Medium = names(n_counts), Value = as.numeric(n_counts)),
            aes(x = Medium, y = min(livedta$meanCW) - 20, label = as.character(Value)),
            color = "black", size = 4, vjust = -0.5) +
  geom_text(data = data_summarized_list$meanCW, 
            aes(x = Medium, y = (0.05 * Max.value) + Max.value, label = groups),
            size = 6, vjust = 0) +
  scale_y_log10() 
...
CellSize_cw_plot <- livedta %>%
  ...
  ylab("Cell diameter") +
  geom_text(data = data.frame(Medium = names(n_counts), Value = as.numeric(n_counts)),
            aes(x = Medium, y = min(livedta$diameter) - 2, label = as.character(Value)),
            color = "black", size = 4, vjust = -0.5) +
  geom_text(data = data_summarized_list$diameter, 
            aes(x = Medium, y = (0.05 * Max.value) + Max.value, label = groups),
            size = 6, vjust = 0)
...
LivingCells_proportion_plot <- ggplot(LivingCells, aes(x = Medium, y = count, fill = cellalive)) +
...
# Save viability chi-squared test's results
v_stats_name <- paste0("v_stats-saved_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".csv")
v_stats_path <- file.path(save_folder, v_stats_name)
write.csv(p_value_matrix, file = v_stats_path, row.names = TRUE, quote = FALSE)

# Save used thresholds as CSV file
